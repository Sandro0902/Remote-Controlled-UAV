...
std::string
getBroadcastData_tcp(DJI::OSDK::Vehicle* vehicle, int responseTimeout)
{
  // Counters
  ...
  std::stringstream bob_the_streamer;
  ...
  // Print in a loop for 2 seconds
    ...
    bob_the_streamer << "Counter = " << elapsedTimeInMs << ":\n"
    << "-------\n"
    << "Flight Status                         = "
    ...
    << "RC Commands           (r/p/y/thr)     = " << rc.roll << ", "
              << rc.pitch << ", " << rc.yaw << ", " << rc.throttle << "\n"
     << "Velocity              (vx,vy,vz)      = " << velocity.x
              << ", " << velocity.y << ", " << velocity.z << "\n"
     << "Attitude Quaternion   (w,x,y,z)       = " << quaternion.q0
